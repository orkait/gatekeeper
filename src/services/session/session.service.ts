...
import { generateId, ok, err, nowMs, hashSHA256, generateRandomToken } from '../shared';
import type { CreateSessionInput, SessionTokens } from './types';
...
export class SessionService {
    ...
    async refreshSession(
        ...
        await this.repository.updateSession(session.id, {
            ...
            expiresAt: nowMs() + this.refreshTokenExpiresIn * 1000,
        });
    ...
    async validateAccessToken(
        ...
        if (!result.valid || !result.payload) {
            return err(result.error || 'Invalid token');
        }
